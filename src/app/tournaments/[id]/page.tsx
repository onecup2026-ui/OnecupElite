...
export default function TournamentDetailPage() {
  ...
  return (
    <div className="bg-background min-h-screen">
      {/* Hero Section */}
      <section className="relative h-[35vh] md:h-[45vh] min-h-[300px] md:min-h-[400px] flex items-end">
        ...
        <div className="container mx-auto px-4 relative z-10 pb-8 md:pb-12">
          <div className="space-y-4 md:space-y-6">
            <Link href="/tournaments">
              <Button variant="ghost" size="sm" className="gap-2 text-white hover:bg-white/10 rounded-full font-bold uppercase text-[9px] md:text-[10px] tracking-widest h-8 md:h-9 border border-white/20">
                <ArrowLeft className="w-3.5 h-3.5 md:w-4 h-4" /> Tournois
              </Button>
            </Link>
            <div className="space-y-2 md:space-y-3">
              <Badge className={cn(
                "uppercase font-bold px-3 py-0.5 md:px-4 md:py-1 text-[9px] md:text-[10px] rounded-full",
                status === "Ouvert" ? "bg-green-500" : "bg-destructive"
              )}>
                {status === "Ouvert" ? "Inscriptions Ouvertes" : "Complet"}
              </Badge>
              <h1 className="text-2xl sm:text-4xl md:text-7xl font-headline font-black text-white uppercase tracking-tighter leading-tight md:leading-none">
                {tournament.name}
              </h1>
            </div>
          </div>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12">
          ...
          <div className="lg:col-span-7 space-y-8 md:space-y-12">
            <div className="space-y-4 md:space-y-8">
              <div className="flex items-center gap-3">
                <div className="h-1 bg-primary w-8 md:w-12 rounded-full" />
                <h2 className="text-xl md:text-2xl font-headline font-black uppercase tracking-tight">Présentation</h2>
              </div>
              <div className="text-muted-foreground text-base md:text-lg leading-relaxed font-medium">
                {tournament.description || "Un tournoi d'exception pour révéler les nouveaux talents."}
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              <Card className="border-none bg-muted/30 p-6 md:p-8 rounded-2xl md:rounded-3xl space-y-4 md:space-y-6 shadow-sm">
                <div className="flex items-center gap-3 text-primary">
                  <Trophy className="w-5 h-5 md:w-6 md:h-6" />
                  <h3 className="font-bold uppercase tracking-tight text-sm md:text-base">Récompenses</h3>
                </div>
                <div className="space-y-3 md:space-y-4">
                  <div className="flex items-center gap-3 md:gap-4">
                    <span className="text-[10px] md:text-xs font-black bg-primary text-white w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-lg">1</span>
                    <p className="text-xs md:text-sm font-bold uppercase">Champion : Trophée + Prime</p>
                  </div>
                  <div className="flex items-center gap-3 md:gap-4">
                    <span className="text-[10px] md:text-xs font-black bg-muted w-5 h-5 md:w-6 md:h-6 flex items-center justify-center rounded-lg">2</span>
              ...
              <Card className="border-none bg-muted/30 p-6 md:p-8 rounded-2xl md:rounded-3xl space-y-4 md:space-y-6 shadow-sm">
                <div className="flex items-center gap-3 text-primary">
                  ...
                  <h3 className="font-bold uppercase tracking-tight text-sm md:text-base">Infos Pratiques</h3>
                </div>
                <div className="space-y-3 md:space-y-4">
            ...
            {tournament.teaserVideoUrl && (
              <div className="space-y-4 md:space-y-6">
                 <h2 className="text-xl md:text-2xl font-headline font-black uppercase tracking-tight">Vidéos</h2>
                 <div 
                  className="aspect-video bg-muted rounded-2xl md:rounded-3xl flex items-center justify-center border group cursor-pointer overflow-hidden relative"
                 ...
                 >
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-3 md:gap-4">
                      <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-primary flex items-center justify-center text-white shadow-xl group-hover:scale-110 transition-transform">
                        <Play className="w-5 h-5 md:w-7 md:h-7 fill-current" />
                      </div>
                      <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-white">Voir le teaser</span>
                    </div>
                 </div>
              </div>
            )}
          ...
          <div className="lg:col-span-5">
            <Card className="sticky top-24 border-none shadow-2xl rounded-2xl md:rounded-3xl overflow-hidden">
              <div className="bg-primary p-4 md:p-6 text-center">
                <h3 className="text-white uppercase font-black tracking-tight text-lg md:text-xl">S'inscrire au tournoi</h3>
              </div>
              <CardContent className="p-6 md:p-8 space-y-6 md:space-y-8">
                {isFull ? (
                  <div className="text-center py-6 md:py-8 space-y-4">
                    ...
                    <p className="font-bold uppercase text-destructive text-sm md:text-base">Complet</p>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest">Les inscriptions sont closes.</p>
                  </div>
                ) : (
                  <form onSubmit={handleLoginAndSubmit} className="space-y-4 md:space-y-6">
                    <div className="space-y-1.5">
                      <Label className="text-[9px] md:text-[10px] uppercase font-bold text-muted-foreground tracking-widest ml-1">Nom de l'équipe</Label>
                      <Input 
                        required 
                        placeholder="Ex: AS Elite" 
                        className="h-11 md:h-12 rounded-lg md:rounded-xl bg-muted/50 border-none focus-visible:ring-primary/20 text-sm" 
                        value={formData.teamName} 
                        onChange={e => setFormData({...formData, teamName: e.target.value})} 
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-[9px] md:text-[10px] uppercase font-bold text-muted-foreground tracking-widest ml-1">Capitaine</Label>
                      <Input 
                        required 
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-[9px] md:text-[10px] uppercase font-bold text-muted-foreground tracking-widest ml-1">Numéro WhatsApp</Label>
                      <Input 
                        required 
                        type="tel" 
                        placeholder="+243 ..." 
                    ...
                    <div className="flex items-start space-x-3 p-3 md:p-4 rounded-xl border bg-muted/20">
                      <Checkbox 
                        id="rules" 
                        ...
                        className="mt-0.5"
                      />
                      <label htmlFor="rules" className="text-[9px] md:text-[10px] font-bold leading-tight uppercase text-muted-foreground cursor-pointer">
                    ...
                    <Button 
                      ...
                      className="w-full h-12 md:h-14 uppercase font-black text-xs md:text-sm bg-primary hover:bg-primary/90 rounded-lg md:rounded-xl shadow-lg transition-all"
                    >
                ...
                <div className="pt-4 border-t flex flex-col gap-2">
                   <Button variant="ghost" onClick={handleShare} className="w-full h-10 rounded-lg md:rounded-xl uppercase font-bold text-[9px] md:text-[10px] tracking-widest gap-2 hover:bg-muted">
